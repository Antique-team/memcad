...
val cons_emp_ind: int -> ind_edge -> n_cons option
...
val build_set_args: int -> graph -> graph * nid list
(* choose the integer parameters weaken type *)
val compute_wk_type: int -> ind_edge -> (n_cons -> bool) -> pars_wk_typ

(* merge two integer weaken type *)
val merge_wktype: int_wk_typ -> int_wk_typ -> int_wk_typ
(* convert types from inductive edge to segment end point *)
val seg_end_wkt: int_wk_typ IntMap.t -> int_wk_typ IntMap.t

(* type a list of integer parameters *)
val type_iargs_ie: ind_args -> int_wk_typ IntMap.t -> int_wk_typ IntMap.t
  -> int_wk_typ IntMap.t
(* compute weaken type for integer parameters in a graph *)
val type_iargs_g: graph ->(int -> int) -> int_wk_typ IntMap.t

(** Management of set variables *)
val setv_add_fresh: bool -> Set_sig.set_par_type option -> graph
  -> int * graph
val setv_add: bool -> int -> graph -> graph
...
val ind_edge_make: string -> nid list -> int -> int ->
  graph -> ind_edge * graph
(* Make a new segment edge with fresh arg nodes *)
val seg_edge_make: string -> nid list -> nid list -> int -> int -> int ->
  graph -> seg_edge * graph
...
val empty_segment_equalities: int -> seg_edge -> PairSet.t * PairSet.t
...
val init_graph_from_roots:
  bool -> (int * int) IntMap.t -> (int * int) IntMap.t
    -> graph -> graph -> graph
